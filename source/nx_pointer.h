...
/* Default pointer id for the cursor (stick / gyro / mouse). Touch events use
 * the lower slots, so `id == NXP_CURSOR_ID` is how a port tells "the cursor
 * moved" from "a finger touched the panel". */
#define NXP_CURSOR_ID 8
...
typedef struct {
  ...
  const char *data_dir;          /* where cursor.png lives, e.g. "sdmc:/switch/supermonkey" */
