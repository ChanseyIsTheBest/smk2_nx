...
static void log_open_locked(void) {
  ...
  /* g_log_path is the RUNTIME log path, e.g. "sdmc:/switch/supermonkey/smk_nx.log"
   * -- resolved by nx_data_root.c from wherever libnative.so actually is, so it
   * is not a compile-time constant. Strip the "sdmc:" mount prefix for the raw
   * fs API, whose paths are relative to the SD filesystem root. */
  const char *path = g_log_path;
  const char *colon = path;
  while (*colon && *colon != ':') colon++;
  if (*colon == ':') path = colon + 1;   /* -> "/switch/supermonkey/smk_nx.log" */
