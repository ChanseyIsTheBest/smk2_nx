...
void so_finalize(so_module *mod) {
  ...
  // svcMapProcessCodeMemory just made load_base inaccessible (its physical pages
  // now live under the executable load_virtbase alias). The .dynsym/.dynstr
  // pointers we cached point into load_base, so rebase them onto load_virtbase
  // (same pages, now readable) -- otherwise so_find_addr*() faults after finalize.
  if (mod->syms)
...
so_module *so_loaded_list(void) { return so_list; }
