...
static void mix_player(Player *p, int32_t *acc, int frames) {
  ...
  for (int i = 0; i < frames; i++) {
    ...
    //
    // RACE SAFETY: the engine can call bq_Clear()/bq_Enqueue() from its own
    // thread at any moment, which mutate p->cur / the queue under p->lock. We
    // must therefore only ever touch p->cur, p->cur_size and the queue while
    // holding p->lock, and mix from a LOCAL snapshot of the buffer pointer --
    // dereferencing the shared p->cur unlocked is a use-after-free/NULL deref.
    ...
    for (;;) {
      ...
      if (n > 0 && (long)p->cur_fpos < n) {
        s = (const int16_t *)p->cur;     // snapshot valid buffer while locked
        fpos = p->cur_fpos;
        SDL_UnlockMutex(p->lock);
      ...
      SDL_UnlockMutex(p->lock);
      if (p->cb)
        p->cb(&p->bq_vt, p->cb_ctx);      // callback runs WITHOUT our lock held
    }
    ...
    SDL_LockMutex(p->lock);
    p->cur_fpos += ratio;                 // advance shared position under lock
    SDL_UnlockMutex(p->lock);
  }
}
...
static void SDLCALL audio_callback(void *ud, Uint8 *stream, int len) {
  ...
  mix_music(acc, frames);      /* BTD5 music path (CustomMediaPlayer) */
