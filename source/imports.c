/* imports.c -- .so import resolution for Bloons Supermonkey
 *              (libnative.so = Ninja Kiwi in-house engine, single module)
 *
 ...
 *
 * Supermonkey's libnative.so imports 380 symbols: a libc subset (shimmed where
 ...
 * in the game dir), __android_log_*, OpenSL ES (emulated in opensles.c), and a
 * networking subset stubbed to fail cleanly so the engine's ad/analytics paths
 * disable themselves. The Java side it calls back into is recreated in
 * ninjakiwi.c / jni_fake.c, not here.
 *
 * Verify coverage against your own copy:
 *     python3 tools/verify_imports.py libnative.so
 *
...
/* ------------------------------------------------------------------------- */
/* import table. 380 undefined symbols in libnative.so, all covered. */
/* ------------------------------------------------------------------------- */


/* ===========================================================================
 * BTD5 supplementary imports. This engine imports a wider set than the
 * BTD5 donor: extra GLES2/EGL entry points and libc, plus networking /
...
/* ---------------------------------------------------------------------------
 * Supermonkey delta -- the 10 imports Bloons TD 5 did not have.
 *
 * The BTD5 donor's table covers 370 of this game's 380 imported symbols. These
 * are the remainder, found by diffing the two .dynsym UND lists. All are plain
 * libc/libm; none needed real shimming, but two are worth a word:
 *
 *   __isfinite / __isnanf  Bionic exports these as real functions. Newlib has
 *                          them only as macros over __builtin_*, so a direct
 *                          symbol reference does not resolve and must be given
 *                          a body here.
 *   tcgetattr / tcsetattr  Terminal control. There is no tty. Returning -1
 *                          (rather than 0) matters: 0 claims success and the
 *                          engine then believes it has a terminal and reads
 *                          back the termios struct we never filled in.
 * ------------------------------------------------------------------------- */
static int   imp_isfinite(double x)  { return __builtin_isfinite(x); }
static int   imp_isnanf(float x)     { return __builtin_isnan(x); }
static int   imp_isdigit(int c)      { return isdigit(c); }
static int   imp_isprint(int c)      { return isprint(c); }
static float imp_logf(float x)       { return logf(x); }
static float imp_powf(float x, float y) { return powf(x, y); }
static char *imp_stpcpy(char *d, const char *s) { size_t n = strlen(s); memcpy(d, s, n + 1); return d + n; }
static char *imp_strtok(char *s, const char *delim) {
  /* Non-reentrant strtok. The engine calls it from one thread only (config
   * parsing at load), so a plain static cursor is correct here; making it
   * thread-local would be misleading about a guarantee bionic's strtok does
   * not actually give. */
  static char *save;
  return strtok_r(s, delim, s ? &save : &save);
}
static int   imp_tcgetattr(int fd, void *t) { (void)fd; (void)t; return -1; }
static int   imp_tcsetattr(int fd, int a, const void *t) { (void)fd; (void)a; (void)t; return -1; }
...
DynLibFunction dynlib_functions[] = {
  ...
  { "glScissor", (uintptr_t)&glScissor_fake },   /* portrait fit: remap the clip rect */
  ...
  /* --- Supermonkey delta (see the note above the helpers) --- */
  { "__isfinite", (uintptr_t)&imp_isfinite },
  { "__isnanf",   (uintptr_t)&imp_isnanf },
  { "isdigit",    (uintptr_t)&imp_isdigit },
  { "isprint",    (uintptr_t)&imp_isprint },
  { "logf",       (uintptr_t)&imp_logf },
  { "powf",       (uintptr_t)&imp_powf },
  { "stpcpy",     (uintptr_t)&imp_stpcpy },
  { "strtok",     (uintptr_t)&imp_strtok },
  { "tcgetattr",  (uintptr_t)&imp_tcgetattr },
  { "tcsetattr",  (uintptr_t)&imp_tcsetattr },
};
