 ...
 *
 * Both cursor paths normalise against a 1080-tall screen. That was written for
 * a landscape 1920x1080 render, but this port renders portrait 1080x1920, so
 * screen_h is 1920 and the normalisation alone already makes the cursor 1.78x
 * bigger than intended -- before the 2.4 base is applied. Halved here, which
 * puts it back to roughly the size it is in landscape ports. */
#define CURSOR_SCALE 0.5f
...
void nxp_draw(void) {
  ...
  if (s_cursor_tex) {
    ...
    glUniform1f(s_u_scale, CURSOR_SCALE * (GLfloat)s_cfg.screen_h / 1080.0f);
    ...
    const GLfloat sc = CURSOR_SCALE * 2.4f * ((GLfloat)s_cfg.screen_h / 1080.0f);
