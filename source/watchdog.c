/* watchdog.c -- stall detector for the BTD5 frame loop.
 *
...
void watchdog_start(so_module *mod) {
  ...
  /* Priority must be in the Horizon range (0x00 highest .. 0x3B lowest for user
   * threads); 0x3f is INVALID and threadCreate fails. The main thread runs at
   * 0x2C, so use 0x2D (just below it) and let the core be unpinned (-2). */
  Result rc = threadCreate(&s_wd_thread, watchdog_main, NULL, NULL, 0x4000, 0x2D, -2);
