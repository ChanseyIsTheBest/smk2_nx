...
// Head of the linked list of currently-loaded modules. Used by the crash
// handler (nx_crash_handler.c) to symbolize a faulting address as name+offset
// even though the loaded .so images are not real OS modules creport can see.
so_module *so_loaded_list(void);
